...
namespace SignalProcessing.Logic.Filters
{
    class ZFilterPass1 : Filter
    {
        ...
        public ZFilterPass1(double r, double frequencyThreshold)
        {
            this.r = r;
        ...
        public override Signal FilterSignal(Signal signal)
        {
            ...
            ComplexNumber z1 = new ComplexNumber(z.real, 0);

            ComplexNumber p1 = new ComplexNumber(r * Math.Cos(fi), r * Math.Sin(fi));
            ComplexNumber p2 = ComplexNumber.conjugate(p1);

            double a0 = ComplexNumber.module(ComplexNumber.divide(ComplexNumber.multiply(ComplexNumber.substract(z, p1), ComplexNumber.substract(z, p2)), ComplexNumber.substract(z, z1)));

            for (int c = 0; c < signal.NumberOfChannels; c++)
            {
                signal.GoToSample(c, 1);
                resultSignal.GoToSample(c, 2);

                int last1 = 0, last2 = 0;
                int lastX = signal.GetSample(c);

                for (int i = 2; i < signal.NumberOfSamples; i++)
                {
                    int x = signal.GetSample(c);
                    int value = (int)(a0 * x + a0 * r * z.real / 9 * lastX + 2 * r * z.real * last1 - r * r * last2);
