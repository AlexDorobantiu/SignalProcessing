...
namespace SignalProcessing.Logic.Filters
{
    class LowPassFilter : Filter
    {
        ...
        public LowPassFilter(int n, double frequencyThreshold)
        {
            this.n = n;
        ...
        public override Signal FilterSignal(Signal signal)
        {
            ...
            for (int k = -(n - 1) / 2; k <= (n - 1) / 2; k++)
            {
                if (k == 0)
                {
                    convolutionKernel[k + (n - 1) / 2] = z;
                }
                else
                {
                    double y = 2 * Math.PI * frequencyThreshold / fe * k;
                    convolutionKernel[k + (n - 1) / 2] = z * (Math.Sin(y) / (y));
                }
            }
